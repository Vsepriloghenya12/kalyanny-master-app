...
export function MixerPage({ content, favoriteMixes, onToggleFavoriteMix, onOpenMix }: MixerPageProps) {
  const [selectedTags, setSelectedTags] = useState<string[]>(['тропики']);

  const matches = useMemo(() => {
    if (!selectedTags.length) return content.mixes;
    return content.mixes.filter((mix) => selectedTags.every((tag) => mix.notes.includes(tag)));
  }, [content.mixes, selectedTags]);
  ...
  return (
    <section className="content-section">
      ...
      <p className="lead-text">Выбери вкусовые ноты и посмотри готовые сочетания. Это первая рабочая версия, дальше сюда можно добавить полноценный конструктор чаши.</p>
